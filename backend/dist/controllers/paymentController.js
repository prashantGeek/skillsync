...
const createCheckoutSession = async (req, res) => {
    try {
        ...
        const existingPurchase = await prisma_1.default.purchase.findFirst({
            where: {
                userId,
                courseId,
                status: 'completed',
...
const stripeWebhook = async (req, res) => {
    ...
    let event;
    try {
        event = stripe.webhooks.constructEvent(req.body, sig, process.env.STRIPE_WEBHOOK_SECRET || 'whsec_123');
    }
    catch (err) {
